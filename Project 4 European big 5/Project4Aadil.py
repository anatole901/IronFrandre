...
plt.pie(pd.DataFrame(score_win).value_counts(), labels = indexes)
plt.show()

#pie chart with the repartition of games played with a specific end score for the winner
plt.pie(pd.DataFrame(score_win).value_counts(), labels = indexes, autopct = '%.0f%%')
plt.title("Winner's number of goals\nrate over all matches")
plt.show()

#Pie chart "Winner's number of goals\nrate over all matches"
plt.pie(pd.DataFrame(score_win).value_counts(), labels = indexes, autopct = '%.0f%%', explode = [1/(len(indexes) - i + 1) for i in range(len(indexes))])
plt.title("Winner's number of goals \n rate over all matches")
plt.show()

#boxplot number of goals by the team playing home
sns.boxplot(x='season', y = 'score_home', data = big5)
plt.title('number of goals by the team playing home')
plt.show()
...
ranks.loc[ranks['season'] == 2017, ['season', 'home', 'total']].head()
ranks.loc[ranks['season'] == 2018, ['season', 'home', 'total']].head()
ranks.loc[ranks['season'] == 2019, ['season', 'home', 'total']].head()
ranks.loc[ranks['season'] == 2020, ['season', 'home', 'total']].head()
ranks.loc[ranks['season'] == 2021, ['season', 'home', 'total']].head()
...
#2017 frequentation des stades
sns.set_theme(style="whitegrid")

attendance = big5.loc[big5['season']==2017,'attendance']
stadium = big5.loc[big5['season']==2017,'venue']
# Draw a scatter plot while assigning point colors and sizes to different
# variables in the dataset
f, ax = plt.subplots(figsize=(6.5, 6.5))
sns.despine(f, left=True, bottom=True)

sns.scatterplot(x= stadium, y= attendance,
                palette="ch:r=-.2,d=.3_r",
                sizes=(1, 8), linewidth=0,
                data=big5, ax=ax)
plt.title('Stadium attendance\n2017')
plt.xticks(stadium, stadium, rotation = 90)


#2018 frequentation des stades
sns.set_theme(style="whitegrid")

attendance = big5.loc[big5['season']==2021,'attendance']
stadium = big5.loc[big5['season']==2021,'venue']
# Draw a scatter plot while assigning point colors and sizes to different
# variables in the dataset
f, ax = plt.subplots(figsize=(6.5, 6.5))
sns.despine(f, left=True, bottom=True)
...
#2019 frequentation des stades
sns.set_theme(style="whitegrid")

attendance = big5.loc[big5['season']==2019,'attendance']
stadium = big5.loc[big5['season']==2019,'venue']
# Draw a scatter plot while assigning point colors and sizes to different
# variables in the dataset
f, ax = plt.subplots(figsize=(6.5, 6.5))
sns.despine(f, left=True, bottom=True)
...
#2020 frequentation des stades
sns.set_theme(style="whitegrid")

attendance = big5.loc[big5['season'] == 2020,'attendance']
stadium = big5.loc[big5['season'] == 2020,'venue']
# Draw a scatter plot while assigning point colors and sizes to different
# variables in the dataset
f, ax = plt.subplots(figsize=(6.5, 6.5))
sns.despine(f, left=True, bottom=True)
...
referee_productivity = round(big5.loc[big5['season']==2021, 'referee'].value_counts()*100/308, 0)

referee_productivity

#linear plot Match percentage refereed
plt.plot( range(7), [list(referee_productivity).count(i) for i in range (7)])
plt.xlabel('Match percentage refereed')
plt.ylabel('Number of referees')
plt.title('Number of referee vs Activity')
plt.show()

#histogram plot Match percentage refereed
sns.displot(referee_productivity)
plt.xlabel('Match percentage refereed')
plt.ylabel('Number of referees')
plt.title('Number of referee vs Activity')
plt.show()

#repartition of our numerical columns in our data set
#figure(figsize=(12, 6))
plt.style.use('classic')
big5.hist()
plt.show()

#violinplot distribution of attendance
ax = sns.violinplot(big5['attendance'])
ax

#heatmap to see the intensity of the correlation between expected goals home and the rality of the score after the game
sns.heatmap(big5[['xg_home','score_home','points_home']].corr())

sns.heatmap(big5[['xg_away','score_away','points_away']].corr())

sns.heatmap(big5[['xg_home','score_home','points_home', 'xg_away','score_away','points_away']].corr())
plt.title("Correlation between expected goals and actual goals\n")
...
#export cleaned data to a new file big5Cleaned.csv
big5.to_csv(r'C:\Users\ABR\Desktop\Projet4_Big_5_League\big5Cleaned.csv')
